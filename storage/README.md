...
- [dust](https://github.com/bootandy/dust) - written in Rust, it runs faster than `du` and has a built in sorting. I find the defaults output w/o using flags confusing to read as it mixes many levels together and sub-sorts them, but it might be OK to others. `dust -F` and `dust -D` appear to be very useful and formatted well.
- [baobab](https://github.com/GNOME/baobab) - analizes partitions quite quikcly and provides a UI to navigate the results based on sub-folder size with easy copying of the paths useful when needing to delete a lot of old models/datasets. I think this tool is only useful on a desktop since it requires GNOME env.
