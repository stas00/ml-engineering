...
- add a rule to [build/SESSION.md](build/SESSION.md) covering assistant-derived techniques. `Reader-visible grounding` catches numbers a reader cannot see, but not a recommendation invented during a session and presented as established practice - which happened on 2026-08-02 with a "run one accelerator per node" method that had no source and a plausible failure mode. The rule should require such content to be labelled untested or verified before it lands.
