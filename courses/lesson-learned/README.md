...
- 1.1. **Fast parallel storage is a game changer for fast model checkpointing.**
...
- 1.2. **Slow shared storage silently kills developer productivity, not just training throughput.**
...
- 1.3. **Python's tens-of-thousands of tiny files can exhaust inodes long before you run out of actual disk space.**
...
- 1.4. **Dataloading directly from cloud storage during training is painful and should be avoided if at all possible.**
...
## 2. Network and Interconnects

- 2.1. **Always double check whether a bandwidth number is unidirectional or bidirectional (duplex) before comparing it to a measurement.**
...
- 2.2. **A slow inter-node network can force your entire training framework choice, not just your throughput ceiling.**
...
- 2.3. **Sometimes an all-reduce benchmark reports a number that seems to exceed the physical wire's theoretical maximum, and that's not a measurement bug - it's SHARP.**
...
- 2.4. **A shared inter-node network makes performance optimization nearly impossible, because you can't tell what changed - your code or your noisy neighbors.**
...
- 8.1. **A large support headcount on a call is not the same thing as real support, and this distinction is worth negotiating into a contract explicitly.**
...
- 8.2. **Accelerators are what fail, overwhelmingly - roughly 95% of hardware failures on a cluster are the accelerators themselves, not CPUs, storage, or network gear.**
...
- 8.3. **The network can quietly steal accelerator memory that you were sold and are paying for, without anyone telling you in advance.**
...
- 8.4. **Provider software stacks can trap you on old, unsupported OS versions, purely because their own internal tooling hasn't caught up.**
...
- 9.1. **Isolate a failing import or component into a one-liner rather than debugging it inside a large, slow-starting program.**
...
- 9.2. **Bisect to find the exact breaking commit rather than manually scanning through hundreds of commits by eye.**
...
- 9.3. **Get a backtrace from a core file to turn a useless, huge traceback into an exact, actionable line of code.**
...
- 9.4. **Backtrace a live, hung process instead of waiting for it to crash or timeout on its own.**
...
- 9.6. **Missing symbols are very often a link-time or library-version problem, not a mysterious runtime bug - and `nm` lets you actually verify what's present.**
