...
6. Re-validate every `as of this writing` clause. The phrase means "true when this section was written" rather than "true when the book was last updated" - which future-proofs the sentence for the author but tells the reader only that the claim may be stale, with no way to tell how stale. There are 21 of them as of 2026-08-04, concentrated in `compute/accelerator/README.md`, `storage/README.md` and `inference/README.md`. Treat each as an item to check, never as a disclaimer that excuses staleness.
7. When one checks out, prefer replacing the phrase with the actual date, since the book already does this elsewhere - `as of 2026-07-31`, `As of 2026-07-28`. A dated claim tells the next pass how old it is; `as of this writing` cannot.
