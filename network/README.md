...
This situation unfortunately makes it extremely difficult to finetune the performance of your training setup. Since every time you run a test the TFLOPs will vary, so how do you do the optimization? One solution is to use cluster namespaces (e.g., Kubernetes namespaces) where each namespace gets a % of resources of the cluster (e.g., 2 namespaces each with 50% of processors & network bandwidth). This way, workloads on namespace "team A" can't interfere with workloads on namespace "team B" because each have untouchable allocated virtual hardware. You can measure TFOPs on a specific namespace, then reverse the math according to the %. In the example case, you just need to multiply your TFOPs namespace measurement by 2: assuming communication == 0.64 secs/iteration and computation == 0.42 secs/interation, now we have communication == 1.28 secs/iteration and computation == 0.84 secs/interation for each namespace, and so in 1.28 secs, only 1 iteration can be done in a namespace, while without namespaces 2 iterations can be done.
